...
export const getBotResponse = (msg) => {
  ...
  if (m.includes('imprimante')||m.includes('impression'))
    return { type:'suggestions', message:"J'ai trouvé des incidents similaires résolus :", suggestions:[
      { id:'INC-245', titre:'Imprimante HP bloquée', solution:"Redémarrer le spooler d'impression via services.msc", nb:8 },
      { id:'INC-189', titre:'Imprimante ne répond pas', solution:'Réinstaller les pilotes depuis le site HP', nb:5 }]}
  if (m.includes('vpn')||m.includes('réseau')||m.includes('wifi'))
    return { type:'suggestions', message:"Incident similaire trouvé :", suggestions:[
      { id:'INC-312', titre:'VPN erreur certificat', solution:'Désinstaller et réinstaller le client VPN', nb:12 }]}
  if (m.includes('email')||m.includes('outlook')||m.includes('mail'))
    return { type:'suggestions', message:"Incident similaire trouvé :", suggestions:[
      { id:'INC-178', titre:'Outlook ne répond pas', solution:'Réparer Outlook via le panneau de configuration', nb:6 }]}
  if (m.includes('moodle')||m.includes('plateforme'))
    return { type:'suggestions', message:"Incident similaire trouvé :", suggestions:[
      { id:'INC-156', titre:'Moodle inaccessible', solution:'Vider le cache navigateur et réessayer', nb:9 }]}
  if (m.includes('pc')||m.includes('ordinateur')||m.includes('lent'))
    return { type:'suggestions', message:"Incident similaire trouvé :", suggestions:[
      { id:'INC-201', titre:'PC lent au démarrage', solution:'Désactiver les programmes au démarrage via msconfig', nb:7 }]}
  return { type:'no_result', message:"Je n'ai pas trouvé d'incident similaire. Voulez-vous créer un ticket pour qu'un technicien vous aide ?" }
}
